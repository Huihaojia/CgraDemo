...
import spinal.core.{Area, AreaRoot, BitCount, ScopeProperty}
...
object PeaScope extends DataBaseScope{

}

//This can be used to globaly store anything in a NaxScope
class PeaThing[T] extends ScopedHandle[T](PeaScope)

//The only purpose of this is to make usercode pretty <3
object PeaParameter {
  def apply[T] = new PeaThing[T]
}

object Global extends AreaRoot {
  val CONFIG_WIDTH = PeaParameter[Int]
  val DATA_WIDTH = PeaParameter[Int]
  val CONFIG_ADDR = PeaParameter[Int]
  val CONNECT_PE_COUNT = PeaParameter[Int]
}
