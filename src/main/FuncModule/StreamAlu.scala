...
class StreamAlu(dataWidth: Int, fifoDepth: Int) extends Component {
  ...
  val streamAluClockDomain = new ClockingArea(streamAluClock) {
    ...
    val opcodeReg2 = Reg(UInt(io.opcodeIn.getWidth bits)) init(0)

    val dirFlag = (opcodeReg2(3 to 4) === 3 & io.opcodeIn(3 to 4) === 2) | (opcodeReg2(3 to 4) === 2 & io.opcodeIn(3 to 4) === 1)

    when(dirFlag) {
      opcodeReg2 := opcodeReg2
    } otherwise {
      opcodeReg2 := io.opcodeIn
    }
